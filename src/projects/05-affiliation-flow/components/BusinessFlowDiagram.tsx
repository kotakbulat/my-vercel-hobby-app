        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', marginTop: '3rem' }}>
          {flow.map((step, i) => (
            <React.Fragment key={i}>
              <div className="glass-card" style={{ width: '300px', textAlign: 'center', padding: '1rem', borderColor: i === 5 ? 'var(--success)' : 'var(--card-border)' }}>
                <h4 style={{ color: i === 5 ? 'var(--success)' : 'white' }}>{step}</h4>
              </div>
              {i !== flow.length - 1 && (
                <div style={{ fontSize: '1.5rem', color: 'var(--primary)', animation: 'float 2s infinite' }}>↓</div>
              )}
            </React.Fragment>
          ))}
        </div>
